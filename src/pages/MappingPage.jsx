...
const MappingPage = () => {
  ...
  const fields = ["ownerId", "conditionType", "vehicleType", "driverSide", "brand", "vehicleModel", 
    "trim", "modelYear", "kmDriven", "vin", "equipmentType", "equipment", "specsType", "fuelType", "bodyType",
     "dimensions","lengthD", "heightD", "widthD", "wheelBase", "wheels", "weight", "suspension", "engineCapacityLitres", "engineCapacityCc", 
     ...
     "driveType", "fuelConsumption", "fuelTankCapacity", "trunkCapacity", "loadingDeckDimensions", "length"
          ,"width", "height", "noOfAxles",
      ...
      "acHeater", "ventilatedSeats", "massageSeats", "powerSeats", "powerTrunk", "keylessEntry", "remoteStart",
       ...
       "pushStartIgnition", "coverImage", "images", "location",  "continent","verificatnio","verifiedBy", "verifiedAt", "status",
         ,"reason","countries","currency", "price", "priceInUsd", "status",
       "stock", "_id","sku","stockType","color","quantity","date","status","productSku"];
  ...
  const handleSubmit = () => {
    const finalObject = {
      products: csvData.map(row => {
        const stockFields = ['sku', 'stockType', 'color', 'quantity', 'date','status','_id'];
        const locationsFields = ['continent', 'countries'];
         const dimensionFields = ['lengthD', 'widthD', 'heightD'];
         const verificatnioFields = ['verifiedAt', 'status', 'reason', 'verifiedBy'];
     //   const product = Object.fromEntries(fields.map(field => [field, row[mappings[field]] || "null"]));
     const product = Object.fromEntries(
      fields
        .filter(field => !stockFields.includes(field))
        .filter(field => !locationsFields.includes(field))
         .filter(field => !dimensionFields.includes(field))
         .filter(field => !verificatnioFields.includes(field))
    ...
    //"[""hammer"", {""type"": ""power"", ""name"": ""drill""}, ""wrench""]"
        ...
        product.stock = [
          ...
          { _id: row[mappings["_id"]] || "null",
            ...
            status : row[mappings["status"]] || "null",
            date: row[mappings["date"]] || "null"
          }
        ];
        product.verificatnio ={
          verifiedBy:row[mappings["verifiedBy"]] || "null",
          verifiedAt:row[mappings["verifiedAt"]] || "null",
          status:row[mappings["status"]] || "null",
          reason:row[mappings["reason"]] || "null",
      } ;
      product.location= 
        ...
        product.dimensions = {
          length: row[mappings["lengthD"]] || "null" ,
          width: row[mappings["widthD"]] || "null",
          height:  row[mappings["heightD"]] || "null"
        };
        product.loadingDeckDimensions={
